...
export async function GET(req: NextRequest) {
  try {
    ...
    logRouteStart('finance-sales', { weekId: weekIdRaw, periodId: periodIdRaw });
    ...
    // Use Order table - same as Sales Report
    ...
    logRouteComplete('finance-sales', perSale.length);
    return NextResponse.json({ byOutlet: out, byOutletRegion, totalActual, avgPotonganPct, totalPotonganAmount: totalPotongan, sales: perSale });
  } catch (error) {
    return NextResponse.json(
      createErrorResponse("build finance sales report", error), 
      { status: 500 }
    );
  }
}
