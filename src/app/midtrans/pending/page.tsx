...
export default async function MidtransPendingPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const whatsappMessage = `Halo Kak, saya belum menyelesaikan pembayaran untuk order ${orderId ? `#${orderId}` : ""}. Mohon bantuannya untuk proses pembayaran.`;

  return (
    <MidtransResultCard
      variant="pending"
      title="Pembayaran Belum Selesai"
      description="Sepertinya kamu menutup halaman Midtrans sebelum proses pembayaran selesai. Tidak apa-apa, kamu bisa lanjutkan kapan saja."
      orderId={orderId}
      grossAmount={grossAmount}
      whatsappMessage={whatsappMessage}
    >
      <ul className="list-disc space-y-1 pl-5">
        <li>Buka kembali tautan pembayaran di email/WhatsApp untuk melanjutkan.</li>
